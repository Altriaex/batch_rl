...
flags.DEFINE_string('agent_name', 'quantile', 'Name of the agent.')
...
def create_logs_for_training(FLAGS):
    folder_name = "_".join(["training_logs", FLAGS.agent_name, FLAGS.method])
    training_log_path = ""
    ...
    preference_reward_file = "_".join([exp_id, game, split, FLAGS.method]) + ".zip"
...
def pack_agents(FLAGS):
    ...
    path, game = osp.split(path)
    exp_base, exp_id = osp.split(path)
    agent_name = "_".join([FLAGS.agent_name, FLAGS.method])
...
if __name__ == '__main__':
    should_be_required = ["original_log_folder", "method", "exp_dir"]
